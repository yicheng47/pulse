...
- [`playback-row-transport-feedback.md`](playback-row-transport-feedback.md) - P2. Transport buttons have no hover/pressed feedback, and previous / next dim-and-restore on every resume because the row treats the engine's transient `Loading` as "transport unavailable".
