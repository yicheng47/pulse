...
- **`pulse-engine` stays UI-agnostic.** No gpui or `pulse-app` types in the engine crate; it must remain drivable from `pulse-cli`. This is also the future process-split seam.
