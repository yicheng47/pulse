...
- **Auto remains policy, not a third segment**: an unpinned device still shows the resolved Shared or Exclusive segment with the `AUTO` tag, and `Reset to Auto` remains. Automatic resolution keeps today's intent: choose Exclusive with the raw integer backend when the hardened integer-path capability passes; choose Exclusive with AUHAL when exclusive native-rate output is useful but the raw integer path is unsafe; otherwise choose Shared.
...
- Visual comparison against the approved Pencil frames for the Devices page and output popover, including pinned and Auto states plus an Exclusive device using each backend.
...
- Open question for Jason: per-track integer refusals (a mono file, or a 32-bit source on a device whose widest container is 24 bits — 78 review §4.5) could fall back to AUHAL-exclusive with a Transparent verdict instead of erroring, which would resolve 78 stage 5 without touching the packer; DSD never falls back. Out of scope until decided.
