...
- **Per-device engine selection** (Jason, 2026-08-31 — the Roon/JRiver model): each device on the Devices page gets an Engine control — `Universal (default)` vs `Bit-perfect (experimental)` — following feature 08's per-device exclusive-mode pattern. Capability-gated: the Bit-perfect option is disabled, with a capability note, unless the device reports signed-integer physical formats and its transport is neither display class (DisplayPort/HDMI) nor Bluetooth. The Stage 1 probe resolved both halves: built-in speakers fall out via their float-only physical formats, while all three DELL displays advertise integer physical formats but refuse the matching integer virtual format. The existing `max_bits_per_channel: Some(_)` capability already answers the format half; Stage 4 must add transport class to the probed and stored capabilities. Start-time virtual-format failure with a clear error remains the backstop for devices whose formats changed since probing, not the primary display gate. Stored per device UID in `settings.json`; changing the active device's engine restarts playback. Hog is mandatory on this engine, so the existing per-device Exclusive control is implied-on under Bit-perfect — the design pass decides whether it locks on or hides, but it must not remain a dead toggle. The controller's `BackendFactory` seam (`controller.rs::spawn`) already abstracts the engine behind `Box<dyn PlaybackBackend>`; the per-device setting picks the factory. Default is **Auto** (Jason, 2026-08-31, superseding the earlier Universal-everywhere default): stored engine `None` = Auto, which resolves to Bit-perfect on devices that pass the transport + format gate and to Universal everywhere else — feature 08's AUTO model; an explicit selection pins and drops the AUTO tag. Interaction rule: Auto never overrides an exclusive-mode pin to shared (Bit-perfect requires hog), so it resolves to Universal there; selecting Bit-perfect explicitly locks Exclusive to REQUIRED. Resolution happens app-side from stored capabilities; the controller always receives a resolved kind. The bit-perfect *wording* in the UI remains gated on the phase-4 DoP pass.
