...
- **AUTO gate parity.** One predicate, in `hal.rs`, decides whether an `AudioStreamBasicDescription` is an integer wire format the engine will accept (linear PCM, signed integer, non-mixable, little-endian, interleaved, container ≤ 32 bits, whole-byte width). The capability probe reports whether the device offers at least one, the stored capabilities carry it (absent → reprobe, as transport did), and `automatic_output_mode` plus the Devices page's `NO INTEGER PATH` tag read it. The existing `max_bits_per_channel` keeps its meaning for the capability line and for Exclusive.
- **Stream-indexed IOProc fill.** `select_integer_format` reports the chosen stream's index (its position in the device's output stream list); `RawSink` carries it; the callback pops the ring only into that stream's buffer, zero-fills every other non-null buffer, and counts underruns only against the selected stream. A disabled stream's null buffer no longer counts as dropout.
...
2. **Gate parity** (`hal.rs`, `device.rs`, app `logic.rs`/`settings.rs`): extract `is_integer_wire_format`, use it from `integer_candidate` and the probe, add the capability field + stored-capability reprobe, wire the resolver and the tag.
