...
`design/pulse-desktop.pen`: screen **`Settings / Update`** (`D9PDB`) — hero with app mark, version chip, status line, and action button, plus the "Check for updates on launch" toggle — and **`Spec — Update States`** (`ADNMS`), which fixes the status line copy and button rendering for all four states (IDLE, CHECKING, AVAILABLE, FAILED). The surface itself is [feature 06](06-settings-surface.md); this feature owns the behavior behind it and depends on 06 landing first.
...
- **Primary surface: the Settings ▸ Update page**, per `Spec — Update States`. The status line and button track the four states; AVAILABLE promotes the button to the accent style and its action opens the release page in the default browser via `NSWorkspace` (the objc2 idiom already used for icon and running-application calls).
- **Secondary surface: the existing notice banner.** A launch check that finds a newer version also raises `PlaybackNotice` / `render_notice` (`playback_row.rs`) — the dismissible banner stage 13 already built — so the update is discoverable without opening Settings. Dismissal is per-launch.
- **When it runs.** Once per launch on a background worker, after the window is up so first paint is never delayed; on demand from the Settings button; and from a **Pulse ▸ Check for Updates…** menu item that opens Settings on the Update section and runs the same path.
...
- **Opt-out.** The "Check for updates on launch" toggle, persisted as a `check-updates.disabled` marker file in the app config directory following the existing one-file-per-preference convention (`app-output-device.uid`). This is Pulse's only network call, and a local-first audiophile player should not phone home with no way to stop it. Document it in `README.md` alongside the privacy note. Turning it off never disables the manual check.
