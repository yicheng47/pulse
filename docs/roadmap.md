...
- **In flight**: feature 78 integer engine hardening — staged plan ([`impls/78-integer-engine-plan.md`](impls/78-integer-engine-plan.md)), stage 1 merged 2026-09-03 (`64d3bc5`, Jason's review); stage 2's predicate rides feature 81 phase 2 (spec + design approved 2026-09-03), then stages 3–4; feature 76 seamless in-track transport (spec filed, mission queued behind 78); feature 77 manual device unlock (spec + Pencil pass in progress, design gate is Jason's).
...
- **Up next**: feature 56 — now-playing page (design pinned, mission-ready); feature 78 stages 2–4 after each merge, stage 5 (mono sources) on Jason's decision; feature 81 simplifies the output control to Shared/Exclusive after 78 lands.
...
| [Feature 78 — integer engine hardening](features/78-integer-engine-hardening.md) | P2 | in flight | Filed 2026-09-02 from the engine review: in-place format change at track boundaries, AUTO gate parity, stream-indexed IOProc fill. Staged 2026-09-03 as five PRs, Jason reviews each: [`impls/78-integer-engine-plan.md`](impls/78-integer-engine-plan.md); review record [`impls/78-integer-engine-review.md`](impls/78-integer-engine-review.md). Stage 1 merged 2026-09-03 (`64d3bc5`); stage 2 folded into feature 81 phase 2; Matrix acceptance of stage 1 is Jason's. |
| [Feature 81 — Shared/Exclusive output modes](features/81-shared-exclusive-output-modes.md) | P2 | designed | Two user-facing modes; Exclusive resolves to the hardened integer backend when safe and AUHAL-exclusive otherwise; Bit-perfect stays a Signal Path verdict. Scheduled after 78; Pencil pass first. Pencil pass approved by Jason 2026-09-03 (two neutral segments, lock glyph on Exclusive, no Auto affordance); implementation waits on 78 stage 2 sequencing. |
