...
- **In flight**: feature 78 integer engine hardening — staged plan ([`impls/78-integer-engine-plan.md`](impls/78-integer-engine-plan.md)), stage 1 merged 2026-09-03 (`64d3bc5`, Jason's review), stage 2 landed inside feature 81 phase 2 (`995e426`), stage 4 merged 2026-09-04 (`3c15d6c`, PR #86); stage 3 waits for a multi-stream device report, stage 5 on Jason's decision; feature 76 seamless in-track transport (spec filed, mission queued behind 78); feature 77 manual device unlock (spec + Pencil pass in progress, design gate is Jason's).
...
- **Up next**: feature 56 — now-playing page (design pinned, mission-ready); feature 78 stage 3 when a multi-stream report arrives, stage 5 (mono sources) on Jason's decision.
...
| [Feature 78 — integer engine hardening](features/78-integer-engine-hardening.md) | P2 | in flight | Filed 2026-09-02 from the engine review: in-place format change at track boundaries, AUTO gate parity, stream-indexed IOProc fill. Staged 2026-09-03 as five PRs, Jason reviews each: [`impls/78-integer-engine-plan.md`](impls/78-integer-engine-plan.md); review record [`impls/78-integer-engine-review.md`](impls/78-integer-engine-review.md). Stage 1 merged 2026-09-03 (`64d3bc5`); stage 2 landed in feature 81 phase 2 (`995e426`, PR #82); Stage 4 (honest hog errors, 10 ms pump cadence) merged 2026-09-04 as `3c15d6c` (PR #86). Matrix acceptance of stages 1, 2, and 4 is Jason's; stage 3 waits for a multi-stream device report; stage 5 on his decision. |
...
| [Feature 87 — DoP output label](features/87-dop-output-label.md) | P3 | planned | Filed 2026-09-04 by Jason: for a DSD source the row's output line reads `DoP 176.4 kHz · <device>` and Signal Path says DoP; PCM unchanged. Copy-only. |
