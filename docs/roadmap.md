...
- **v0.3.3** (2026-09-04) — Album artist link ([84](features/archive/84-album-artist-link.md)); DoP named on the row and Signal Path, DSF/DFF album badges ([87](features/archive/87-dop-output-label.md)); feature 78 stage 4 — honest hog errors, 10 ms pump cadence (`3c15d6c`); 78 closed with its leftovers in [89](features/89-integer-engine-followups.md).
...
| [Feature 84 — album page artist link](features/archive/84-album-artist-link.md) | P2 | done | Filed 2026-09-03 by Jason; merged 2026-09-04 as `704178c` (PR #92): the header's artist name opens the artist page through the existing Artists route, plain text when no artist row resolves. Shipped in v0.3.3. |
...
| [Feature 87 — DoP output label](features/archive/87-dop-output-label.md) | P3 | done | Filed and merged 2026-09-04 (`3c5804f`, `c403993`): the row reads `DoP · <device>` for a DSD source, Signal Path `DoP 24/176.4 integer`, the album badge gets its DSF/DFF container. Shipped in v0.3.3. |
