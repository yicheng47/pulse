...
- **Just landed**: v0.3.2 (2026-09-03) — feature 81 Shared/Exclusive output modes (`995e426`, `3e17bb2`) and feature 78 stages 1–2 (in-place format change at track boundaries `64d3bc5`, the integer-wire predicate behind the Exclusive resolver `995e426`). Filed the same day: features 84 (album artist link) and 85 (disc headers), both recorded only.
- **In flight**: nothing running; feature 87 DoP output label merged 2026-09-04 (`3c5804f`, `c403993`) and waits for the next release; feature 78 closed 2026-09-04 with stages 1, 2, and 4 shipped — its two untriggered items are feature 89; feature 76 seamless in-track transport (spec filed, mission queued behind 78); feature 77 manual device unlock (spec + Pencil pass in progress, design gate is Jason's).
...
- **Up next**: feature 56 — now-playing page (design pinned, mission-ready); feature 84 (artist link, no design gate); feature 89 when a multi-stream report or Jason's mono decision arrives.
...
- **v0.3.2** (2026-09-03) — Shared and Exclusive output modes ([81](features/archive/81-shared-exclusive-output-modes.md)): Exclusive resolves to the integer engine on a safe integer path and to AUHAL-exclusive otherwise, Bit-perfect is a Signal Path verdict rather than a switch, `bitPerfect` preferences migrate; feature 78 stages 1–2 — in-place format change at track boundaries (`64d3bc5`), the integer-wire predicate gating the resolver (`995e426`). v0.3.1 (2026-09-03) carried DSD over DoP, toasts, scan performance, and the quit release fix.
...
| [Feature 77 — manual device unlock](features/77-manual-device-unlock.md) | P2 | designing | Filed 2026-09-02 from Jason's ask; resolves the pause-release question. Pencil pass drafted, awaiting Jason's design approval; his 2026-09-03 refinement (a lock button on the active-device card, re-lock only on the next play) is folded into the spec. |
| [Feature 78 — integer engine hardening](features/archive/78-integer-engine-hardening.md) | P2 | done | Filed 2026-09-02 from the engine review; staged as PRs under Jason's review ([`impls/78-integer-engine-plan.md`](impls/78-integer-engine-plan.md), record [`impls/78-integer-engine-review.md`](impls/78-integer-engine-review.md)). Stage 1 `64d3bc5` and stage 2 `995e426` (inside feature 81 phase 2) shipped in v0.3.2; stage 4 (honest hog errors, 10 ms pump cadence) `3c15d6c` on 2026-09-04. Closed 2026-09-04; stages 3 and 5 moved to feature 89. Matrix acceptance of the shipped stages is Jason's. |
| [Feature 81 — Shared/Exclusive output modes](features/archive/81-shared-exclusive-output-modes.md) | P2 | done | Two user-facing modes; Exclusive resolves to the hardened integer backend when safe and AUHAL-exclusive otherwise; Bit-perfect stays a Signal Path verdict. Pencil pass approved by Jason 2026-09-03 (two neutral segments, lock glyph on Exclusive, no Auto affordance). Phase 2 (stored model, resolver, DoP gate on the confirmed engine kind; PR #82) merged 2026-09-03 as `995e426`; phase 3 (the approved surface: neutral segments, lock glyph on Exclusive, no Auto affordance; PR #83) merged 2026-09-03 as `3e17bb2`; phase 4 docs landed the same day. Shipped in v0.3.2 (2026-09-03); the Shared switch was hardware-checked, #81 closed. |
...
| [Bug — volume lock lags the switch to Exclusive on an integer path](bugs/volume-lock-lags-bit-perfect-switch.md) | P2 | planned | Filed 2026-09-03 by Jason: the slider stays live after choosing Exclusive on the Matrix while paused/idle because `VolumeStateChanged(Fixed)` is emitted only from a backend start; fix direction is a provisional domain on `OutputDeviceChanged`. |
| [Feature 84 — album page artist link](features/84-album-artist-link.md) | P2 | planned | Filed 2026-09-03 by Jason: the header's artist name opens the artist page via the existing Artists route; plain text when no artist row exists. Small; no Pencil pass proposed. |
| [Feature 85 — album page disc headers](features/85-album-disc-headers.md) | P2 | planned | Filed 2026-09-03 by Jason: `Disc N` header rows when an album's tracks span discs; the tag is already scanned and the query already orders by disc. Recorded only (Jason, 2026-09-03); Pencil pass on the album track table when scheduled. |
| [Feature 87 — DoP output label](features/87-dop-output-label.md) | P3 | merged | Filed and merged 2026-09-04 (`3c5804f`, `c403993`): the row reads `DoP · <device>` for a DSD source, Signal Path `DoP 24/176.4 integer`, the album badge gets its DSF/DFF container. Ships in the next release; in-app look is Jason's. |
| [Feature 88 — eyebrow text size](features/88-eyebrow-text-size.md) | P3 | planned | Filed 2026-09-04 by Jason: sidebar section labels, page eyebrows, and table column headers move from 10 px `CAPTION` to a new 11 px `EYEBROW` token; badges stay. Pencil pass first. |
| [Feature 89 — integer engine follow-ups](features/89-integer-engine-followups.md) | P3 | planned | Opened 2026-09-04 as #78 closed: stream-indexed IOProc fill (parked until a multi-stream device report) and mono sources (Jason's decision). |
