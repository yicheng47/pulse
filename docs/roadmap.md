...
- **In flight**: feature 78 integer engine hardening — staged plan ([`impls/78-integer-engine-plan.md`](impls/78-integer-engine-plan.md)), stage 1 merged 2026-09-03 (`64d3bc5`, Jason's review), stage 2 landed inside feature 81 phase 2 (`995e426`), stages 3–4 next; feature 81 Shared/Exclusive output modes — phase 2 merged 2026-09-03 (`995e426`, PR #82), phase 3 (the approved surface) in a codex-crew mission, then phase 4 docs and Jason's Matrix smoke pass; feature 76 seamless in-track transport (spec filed, mission queued behind 78); feature 77 manual device unlock (spec + Pencil pass in progress, design gate is Jason's).
...
| [Bug — volume lock lags the switch to Bit-perfect](bugs/volume-lock-lags-bit-perfect-switch.md) | P2 | planned | Filed 2026-09-03 by Jason: the slider stays live after choosing Bit-perfect while paused/idle because `VolumeStateChanged(Fixed)` is emitted only from a backend start; fix direction is a provisional domain on `OutputDeviceChanged`. |
