...
- `script/dop_pack.py` — the stage-5 DFF packer is the reference implementation; the Rust packer must match it byte-exactly. Fixtures (small hand-built DSF/DFF headers + known bit patterns, committed as tiny files, no real music in the repo) are generated by `script/generate_dsd_fixtures.py`, written during the build — `dop_pack.py` has no DSF/DFF writer.
...
1. **Parsers + packer** (engine only): `decode_dsd.rs` with byte-exact fixtures (DSF bit-reversal, DFF interleave, marker alternation, seek alignment, DST refusal); factory routing. `make verify` green, no app change. — **Landed in `f50e040`.**
2. **Library**: walk + metadata + repo rate fields + quality badge; a scanned DSD album browses like any other. — **Landed in `f50e040`**, with the refusal gate pulled forward from phase 3 (review call: scan indexing must never land ungated). The gate's four user-facing strings are **placeholder copy**, awaiting phase 3's design.
3. **Gating UI**: Pencil pass for the unplayable-track treatment (dimming, badge, copy — standing rule: no frontend surface before a design exists), then replace the phase-2 placeholder strings with the designed copy.
...
- **2026-08-31** — Filed as feature 33, gated on the stage-5 DoP acceptance pass.
- **2026-09-01** — Renumbered to 71 (issue-number alignment). Impl note written; mission on `codex peer` (01M1DHYD7CNSBC9A09DBQ9WCQD): phases 1–2 built and review-clean in ~55 minutes, two rounds per phase. Review caught an MSB-first DSF bit-order bug (valid markers, garbage audio) and two gate-bypass routes; the refusal moved to the `send_command` choke point. Merged to `main` as `f50e040` — hardware test deferred, device at home.
- **Next, in order**: phase 3 Pencil pass (unplayable-track treatment; replace the four placeholder gate strings) → phase 4 Matrix acceptance, which closes [#71](https://github.com/yicheng47/pulse/issues/71) → feature 56 (now-playing page, design pinned) next in M4.
