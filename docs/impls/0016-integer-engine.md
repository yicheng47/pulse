...
Tactical plan for [feature 32](../features/32-integer-engine.md) (milestone M3 → v0.3.0). Read-only reference clones: `~/repos/audio/{mpv,MPD,SFBAudioEngine}` — GPL sources are ideas only, no code.
...
- **`integer_engine.rs`** — `IntegerEngine`, mirroring `Engine`'s surface so the backend wrapper is mechanical:
  ...
  - `set_format`: pick the integer physical format (≥ source bits, prefer exact width, then smallest sufficient container); set physical; set virtual = physical; build the `IntPacker`; size the ring at 4s × device bytes-per-frame.
...
- Controller: `EngineKind { Universal, BitPerfect }`; `PlaybackCommand::SetOutputDevice` gains the kind (protocol change — update app dispatch and controller tests); `spawn` takes the initial kind; factory selects `EngineBackend` vs `IntegerBackend`. Runtime start failure on a device that advertised integer support → clear `Error` event, no silent float fallback.
...
- Devices page: an Engine control per device row — enabled only when the device's stored capabilities include integer formats; disabled state carries the "no integer formats" note. **Pencil pass first** (small: one control + one disabled state).
...
DoP test on the Matrix: DSD64-in-DoP FLAC through the integer engine → display flips to DSD64. Repeat plain PCM at 44.1/16 and 96/24 watching the rate display. Record results in the spec; only a pass permits any bit-perfect wording in the UI. Also verify teardown: after quitting Pulse, Audio MIDI Setup shows the device's prior formats restored.
...
- Manual: engine switch restarts cleanly; speakers/Bluetooth show the disabled control; volume slider hardware-routed or disabled; pause clicks are expected and documented.
