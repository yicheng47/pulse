...
- `settings.rs`: `StoredDevicePreferences` gains `engine: Option<StoredEngineKind>` (serde, `None` = **Auto**), accessors beside `exclusive_mode_override`.
- **Auto resolution (Jason, 2026-08-31)**: `None` resolves to Bit-perfect when the stored gating signal passes AND exclusive mode is not pinned shared; Universal otherwise. An explicit stored value pins (AUTO tag drops). Resolution lives app-side — the controller always receives a resolved `EngineKind`. Unit-test the resolution matrix: gate × exclusive-pin × explicit-pin.
