...
- **2026-09-03** — Feature 81 (Shared/Exclusive modes) filed; its phase 1 consumes stage 2's `integer_wire_formats` plumbing and rewrites the same resolver, so stage 2 and 81 phase 1 are sequenced, never concurrent — whichever runs first carries the predicate. 81's fallback decision may resolve stage 5 (mono) without a packer change.
