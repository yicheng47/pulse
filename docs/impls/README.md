...
- [`78-integer-engine-plan.md`](78-integer-engine-plan.md) - feature 78's staged build plan: five stages, each one branch + one PR into `main` + one `codex-crew` mission, merged only after Jason's personal review; stage 5 (mono sources) waits on his decision.
