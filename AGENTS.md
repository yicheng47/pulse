...
- **Track features in GitHub issues + local specs (Jason, 2026-09-01; supersedes the 08-29 local-docs-only rule).** Milestones and build order live in `docs/roadmap.md` — a fresh session starts at its `## Now` section for current state. Features are specs in `docs/features/` with a GitHub issue filed first; **the issue number is the spec number** (`feature` skill has the flow). Bugs remain notes in `docs/bugs/`, no issue. All work carries a P0–P3 priority. Crews report deferred work in their final Runner message and the human adds the row. Durable product and architecture decisions belong in `docs/product/` and `docs/arch/`. User reports arrive as GitHub issues on this repo: reply and close there, and carry the engineering work into `docs/bugs/`. Do not recreate a global `docs/impls/IMPL_LOG.md`; journey logs live per milestone (e.g. `docs/impls/archive/bit-perfect/IMPL_LOG.md`) or as a `## Log` section in the feature's impl note, maintained by the lead session — crews report in their Runner mission and do not edit them.
- **Music only; PCM only inside the engine.** DSD rides as DoP-packed PCM on the bit-perfect engine ([feature 71](docs/features/71-dsd-over-dop.md), Jason 2026-08-31) — no DST, no native/raw DSD, no DSD→PCM conversion. No video, no streaming integration. Don't scope-creep these back in.
...
- **Bit-perfect claims are scoped to the integer engine only.** The raw-HAL integer path (shipped v0.3.0 as the Bit-perfect output mode; since v0.3.2 it is the engine behind Exclusive on devices with a safe integer path — feature 81) is DoP-validated on hardware (2026-09-01) and may be called bit-perfect. The AUHAL/Universal path still may not — its honest ceiling remains "samples leave Pulse untouched"; Core Audio owns the conversion past the float32 client boundary.
