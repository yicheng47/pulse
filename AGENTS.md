...
- **Two repos, one codebase.** This private repo is `yicheng47/pulse-src` (local checkout `~/repos/yicheng47/pulse-src`). The public `yicheng47/pulse` repo holds only a README, the releases, and customer-facing issues — never push code, design files, or docs there. Anything a user downloads (DMG, appcast) must be on the public repo, because private-repo release assets require authentication.
- **Track unfinished work in GitHub issues, not a session log.** Engineering issues and feature gaps live in this repo (`gh` defaults to it) with a priority label; durable product and architecture decisions belong in the relevant docs. Customer reports arrive on the public repo — triage them there with `--repo yicheng47/pulse` and file the engineering issue here with a link back. Do not recreate `docs/impls/IMPL_LOG.md`.
