...
#[cfg(any(test, all(target_os = "macos", feature = "updater")))]
const PULSE_UPDATE_ERROR_DOMAIN: &str = "com.wycstudios.pulse.updater";
...
fn dev_available() -> Option<UpdateInfo> {
    std::env::var("PULSE_DEV_UPDATE_AVAILABLE")
        .ok()
...
mod native {
    ...
    use objc2::{
        AnyThread, DefinedClass, MainThreadMarker, MainThreadOnly, define_class, extern_class,
    ...
    fn background_check_declined_error() -> Retained<NSError> {
        let domain = NSString::from_str(PULSE_UPDATE_ERROR_DOMAIN);
        let description = NSString::from_str("Pulse recorded an update during a background check.");
        let user_info =
    ...
    impl NativeUpdater {
        pub(super) fn new(updater: WeakEntity<Updater>, cx: AsyncApp) -> Self {
            ...
            let automatically_checks = match preferences::take_legacy_update_check_preference() {
                Ok(Some(enabled)) => enabled,
                Ok(None) => controller.updater().automatically_checks_for_updates(),
                Err(error) => {
...
mod tests {
    ...
    fn found_update_sets_available_version() {
        ...
        apply_available_transition(&mut available, UpdateTransition::Found("0.9.9".into()));

        assert_eq!(available, Some(UpdateInfo::new("0.9.9")));
    }

    #[test]
    fn duplicate_found_update_does_not_change_available_state() {
        let mut available = Some(UpdateInfo::new("0.9.9"));

        assert!(!apply_available_transition(
            &mut available,
            UpdateTransition::Found("0.9.9".into()),
        ));
    }
    ...
    #[test]
    fn pulse_decline_abort_preserves_available_update() {
        let mut available = Some(UpdateInfo::new("0.9.9"));

        if let Some(transition) = transition_for_abort(PULSE_UPDATE_ERROR_DOMAIN) {
    ...
    #[test]
    fn pulse_decline_does_not_renotify_same_found_update() {
        let mut available = None;
        assert!(apply_available_transition(
            &mut available,
            UpdateTransition::Found("0.9.9".into()),
        ...
        assert_eq!(transition_for_abort(PULSE_UPDATE_ERROR_DOMAIN), None);
        assert!(!apply_available_transition(
            &mut available,
            UpdateTransition::Found("0.9.9".into()),
        ));
