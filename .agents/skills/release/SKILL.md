...
8. Publish: `gh release edit v{version} --repo yicheng47/pulse --draft=false --latest --notes "..."` — the `--repo` flag is required because `gh` in this checkout defaults to the private `pulse-src`. After publishing, confirm `https://github.com/yicheng47/pulse/releases/latest/download/appcast.xml?cb=1` reports the new version; the un-busted alias can lag a few minutes behind on GitHub's edge cache. If this release is the first exercise of a changed signing/packaging path, hand the draft to the user for a quarantined-DMG Gatekeeper smoke test (download through a browser, mount, launch) before publishing instead.
